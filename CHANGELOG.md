...
### 数据根设置闭环
- `~/.config/anote/config` 重新定义为**数据根定位指针**（仅一行 `data_dir=`）；完整配置仍全部位于 `<数据根>/.anote/config`
- `Config.save()` 自动同步定位指针；`anote migrate --to` 后新数据根可被后续所有命令发现
- **TUI 设置页**：直接修改“数据目录”后按保存，先保存其余设置，再自动执行迁移（含 `.git` 与 `.anote` 配置），成功后当前会话立即切换数据根
- `ANOTE_DATA` 仍是最高优先级与测试隔离硬边界；旧完整配置首次读取时自动迁入数据根并收缩为指针

### P2：检索性能
- BM25 重构为**倒排索引**：查询只扫描命中词 postings，复杂度与命中项相关，不再对全部文档打分
- BM25 索引持久化到 `.semantic/bm25.json`（含 chunks 签名校验），后续检索进程免重建
- 混合检索候选集改为“向量 top-K ∪ BM25 top-K”后再融合排序；`(path, text)` 稳定映射替代 O(n²) `chunks.index`
- `SemanticService` 嵌入模型懒加载并缓存，评测等多查询场景不再重复初始化

### P2：命令注册表
- 新增 `src/anote/commands.py`：全部命令的 name/case/syntax/help/example 单一来源
- TUI 命令面板与帮助页改为消费注册表
- 新增 `tests/test_commands.py`：校验注册表与 `anote` 入口 case 一致

### 修复
- `anote ai` 白名单分发使用 `shlex` 正确解析带引号参数
- 设置页迁移失败时也会刷新界面；迁移期间离开屏幕不再抛异常
- TUI 帮助/命令面板在干净环境可直接导入（不再依赖 import 顺序）
...
- **`anote md <文件> [--watch]`**：终端实时 Markdown 渲染（纯 stdlib 零依赖；ANSI 彩色；1s 轮询保存即刷新；无浏览器/无弹窗/无杂乱）
- **vimrc F5 改为终端分屏实时预览**（`:vert term anote md % --watch`）；F6 浏览器备用
- mdview 自动跳过 LaTeX 注释/命令（兼容历史文件）
