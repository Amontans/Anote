...
notes new 数学/代数 "标题"          # 新建笔记（自动 META 模板）
notes ask "关键词"                  # 问答（grep 模式）
notes ask --semantic "自然语言问题"  # 问答（语义模式）
notes commit "说明"                 # 提交（自动索引+自检）
notes review --days 7              # 周回顾
notes book "书名" && notes book-build "书名"   # 开写/编译教科书
notes backup                        # 提交+推送远程
```
...
### 用户提问时（答疑）
1. 知识库内的问题 → 先跑 `notes ask --semantic "<问题>"`；语义结果不足时 `notes ask "<关键词>"`；仍不足再直接读 `src/` 下相关文件。
...
- **写教科书**：`notes book/chapter/book-build` 全流程

### 红线
- 不直接修改 `src/` 笔记的语义内容（可经 `notes new` 建模板、经用户同意后填充）。
- 不破坏 INTERFACES.md 定义的任何格式。
- 所有变更经 `notes commit` 提交（pre-commit 自动索引+自检）。
...
| 路径 | 作用 | 怎么用 |
|------|------|--------|
| `src/<学科>/<分支>/*.tex` | **唯一真相源**：学习笔记（META 头） | `notes new 学科/分支 "标题"` 起步，VS Code/Vim 写 |
...
| 症状 | 修复 |
|------|------|
| check 报错 | 按提示处理（缺 META→加头；未登记→notes index；PDF 孤儿→补队列行或删文件） |
| 语义问答提示未建索引 | `notes index-semantic` |
| 误删文件 | `git log --oneline` → `git checkout <commit> -- <file>` |
| 语义缓存坏了 | `rm -rf .semantic && notes index-semantic` |
