...
- **v1.17 P2 服务层收尾**：backup/restore/preview/mdview/web 逻辑下沉 services；补服务层回归测试；完成任意项目路径/数据根路径（含空格与中文）可移植性实测。

- **v1.16 数据根设置闭环 + P2 起步**：TUI 设置中修改数据目录保存即自动迁移（配置随迁，定位指针更新）；BM25 倒排索引持久化、混合检索候选集并集、嵌入模型缓存；命令注册表单一来源（TUI/测试消费）。

- **v1.15 可移植性校正**：所有用户数据（配置/日志/MCP注册/迁移日志/默认备份/预览）收进数据根 `.anote/`；项目路径自动推导（`anote`/`setup.sh`/systemd 模板）；`anote new` 防覆盖+LaTeX 转义；`read`/`paper-read` 链路修复；MCP 离线可用；check 8 项+`--strict`；TUI 测试自包含。项目仓库无用户数据，可直接上传 GitHub。
...
### 已补齐（本轮"由基本到上层"）
- 基础：setup.sh 分层安装（轻量化落地）
- 协议：MCP client（消费任意外部工具）
- 应用：M4 标注导入 + M5 文档语义索引层
