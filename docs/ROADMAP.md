...
### v0.9「检索增强」（对标 Obsidian 标签/搜索）
| 功能 | 参考 | 价值 | 成本 |
|------|------|:---:|:---:|
| META 标签浏览/过滤面板（按 学科/标签 筛选笔记） | Obsidian 标签 | 高 | 低 |
| TUI 全文搜索页（rg 集成，结果+行号+跳转） | Obsidian 搜索/Joplin | 高 | 中 |
| 笔记模板系统（`templates/` 多模板选择） | Obsidian 模板 | 中 | 低 |
| 每日笔记（自动生成日期页 + 当日队列/回顾入口） | Obsidian daily notes | 中 | 低 |
...
| 功能 | 参考 | 价值 | 成本 |
|------|------|:---:|:---:|
| **TUI 内 AI 问答面板**（接 DeepSeek：问知识库直接 TUI 内答，带引用） | Pi 对话 | 高 | 中 |
